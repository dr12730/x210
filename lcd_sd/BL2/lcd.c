...
{
	lcd_draw_vline(400, 0, 480, RED);
	lcd_draw_hline(0, 799, 200, GREEN);
	delay();
}
...
void draw_ascii(unsigned int x, unsigned int y, unsigned int color, unsigned char *str)
{
	int i;  
	unsigned char *ch;
    for (i=0; str[i]!='\0'; i++)  
...
void lcd_test(void)
{
	...
	delay();
	test_draw_line();
	draw_circular(400, 200, 50, RED);
	lcd_draw_background(WHITE);
	draw_ascii(50, 50, RED, (unsigned char*)"Welcome to my house!");
